...
double FATIMA_Energy_Calibration::Calibrate(double E,int det_id){
    ...
    //for(int i = 1;i < 5;++i) cout << "det " << det_id << " coeff " << calib_coeffs[det_id][i] << endl;
    return Energy_c;
}
//----------------------------------------------------------
//
...
if (!have_gm_list){
   ...
   if (tslow <= ts_minutes && tshigh > ts_minutes) {
     ...
     if(!foundit) {
       printf("WARNING: FATIMA, no matching gainmatching parameter file found. Not updating parameters!\n");
       do_gm=0;
