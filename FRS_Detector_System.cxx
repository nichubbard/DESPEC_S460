...
///==================================================================================///
  ...
  if(2== id->vftx_s2pos_option){//TPCX is used
   ...
   // Calculation of velocity beta and gamma
  id_vftx_beta_2141   =  ( id->vftx_length_2141 / vftx_tof2141_calib) / speed_light;
  id_vftx_beta_2241   =  ( id->vftx_length_2241 / vftx_tof2241_calib) / speed_light;
  //printf("tof2141 %02f beta %02f\n",vftx_tof2141_calib,id_vftx_beta_2141);
    id_vftx_gamma_2141  = 1./sqrt(1. - id_vftx_beta_2141*id_vftx_beta_2141);
    id_vftx_gamma_2241  = 1./sqrt(1. - id_vftx_beta_2241*id_vftx_beta_2241);
...
//   float mean_brho_s2s4 = 0.5*( frs->bfield[2] + frs->bfield[3] );
  ...
  if( -200<temp_s4x && temp_s4x<200. && -200.< temp_sci21x && temp_sci21x<200.){
    ...
    if(0.0 < id_vftx_beta_2141 && id_vftx_beta_2141 < 1.0){
        id_vftx_aoq_2141 = mean_brho_s2s4 *( 1. + id_vftx_delta_2141   ) * temp_tm_to_MeV / (temp_mu * id_vftx_beta_2141 * id_vftx_gamma_2141);
        id_vftx_aoq_2141_corr = id_vftx_aoq_2141 - id->a2AoQCorr * id_a2;  //correction for id_a2, JK 16.9.11
  ...
  if( -200<temp_s4x && temp_s4x<200. && -200.< temp_sci21x && temp_sci21x<200.){
    ...
    if(0.0 < id_vftx_beta_2241 && id_vftx_beta_2241 < 1.0){
        id_vftx_aoq_2241 = mean_brho_s2s4 *( 1. + id_vftx_delta_2241   ) * temp_tm_to_MeV / (temp_mu * id_vftx_beta_2241 * id_vftx_gamma_2241);
        id_vftx_aoq_2241_corr = id_vftx_aoq_2241 - id->a2AoQCorr * id_a2;  //correction for id_a2, JK 16.9.11
    //printf("aoq %02f aoq_corr %02f\n",id_vftx_aoq_2141,id_vftx_aoq_2141_corr);
      }
  }
  ...
  // calculation of dE and Z
  // from MUSIC41
  if( (de[0] >0.0)  && (id_vftx_beta_2141>0.0) && (id_vftx_beta_2141<1.0)){
    Double_t power = 1., sum = 0.;
    for (int i=0;i<4;i++){
      sum += power * id->mhtdc_vel_a_music41[i];
      power *= id_vftx_beta_2141;
    }
    id_vftx_2141_v_cor = sum;

    if (id_vftx_2141_v_cor > 0.0){
      id_vftx_2141_z = frs->primary_z * sqrt(de[0]/id_vftx_2141_v_cor);
      //printf("z %02f\n",id_vftx_2141_z);
   }
  }
    if( (de[0] >0.0)  && (id_vftx_beta_2241>0.0) && (id_vftx_beta_2241<1.0)){
    Double_t power = 1., sum = 0.;
    for (int i=0;i<4;i++){
      sum += power * id->mhtdc_vel_a_music41[i];
      power *= id_vftx_beta_2241;
    }
    id_vftx_2241_v_cor = sum;

    if (id_vftx_2241_v_cor > 0.0){
      id_vftx_2241_z = frs->primary_z * sqrt(de[0]/id_vftx_2241_v_cor);
      //printf("z %02f\n",id_vftx_2241_z);
   }
  }
...
void FRS_Detector_System::get_Event_data(Raw_Event* RAW){
    ...
    RAW->set_DATA_ID_VFTX(id_vftx_beta_2141, id_vftx_gamma_2141,id_vftx_aoq_2141,id_vftx_aoq_2141_corr,id_vftx_2141_z,id_vftx_2141_v_cor,id_vftx_beta_2241,id_vftx_gamma_2241,id_vftx_aoq_2241,id_vftx_aoq_2241_corr,id_vftx_2241_z,id_vftx_2241_v_cor);
